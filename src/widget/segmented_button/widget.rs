    ...
    Message: 'static + Clone,
{
    ...
    fn overlay<'b>(
        ...
        // Context menu overlay (upstream: per-tab context menu via menu[0].children[i])
        if let Some(entity) = state.show_context {
            let found: Option<(Rectangle, usize)> = self
                .variant_bounds(state, layout.bounds())
                .filter_map(|item| match item {
                    ItemBounds::Button(e, bounds) => Some((bounds, e)),
                    _ => None,
                })
                .enumerate()
                .find_map(|(i, (bounds, e))| {
                    if e == entity { Some((bounds, i)) } else { None }
                });

            if let Some((mut bounds, i)) = found {
                assert!(
                    self.context_menu
                        .as_ref()
                        .is_none_or(|m| m[0].children.len() == self.model.len())
